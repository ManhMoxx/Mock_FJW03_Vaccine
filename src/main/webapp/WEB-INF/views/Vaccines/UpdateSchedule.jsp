...
<body>
	<div class="mt-4" style="padding: 15px;">
		<f:form action="${pageContext.request.contextPath}/updateschedule"
			method="post" modelAttribute="schedule">
			<div>
				<f:input path="injectionSchedule" type="hidden"/>		
			</div>
			<div class="row">
				<div class="col-md-4 mb-3">
					<label for="my-select">Vaccine<span style="color: red;">(*)</span>:
					</label> <select id="my-select" class="form-control"
						name="vaccine.vaccineId" required>
						<option value="">~ Select Vaccine</option>
						<c:forEach var="v" items="${listVaccine}">
							<option value="${v.vaccineId}"
								<c:if test="${v.vaccineId eq schedule.vaccine.vaccineId}">selected</c:if>>${v.vaccineName}</option>
						</c:forEach>
					</select>
				...
				<div class="col-md-4 mb-3">
					...
					<f:input type="date" class="form-control" id="from"
						path="startDate" />
				...
				<div class="col-md-4 mb-3">
					...
					<f:input type="date" class="form-control" id="to" path="endDate" />
			...
			<div class="row">
				<div class="col-md-4 mb-3">
					...
					<f:input type="text" class="form-control" id="place" path="place"
						value="" />
					<f:errors path="place" cssClass="error" />
				</div>
				<div class="col-md-8 mb-3">
					<label for="note">Note:</label>
					<f:input type="text" class="form-control" id="note"
						path="description" />
				...
				<div class="col-md-12">
					...
					<button type="button" class="btn btn-warning">
						<a style="text-decoration: none; color: white;"
							href="${pageContext.request.contextPath}/listschedule">Cancel</a>
					</button>
				</div>
			</div>
		</f:form>
